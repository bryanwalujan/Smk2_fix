...
@section('content')
    <div class="max-w-md mx-auto bg-white p-6 rounded-lg shadow-sm">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Edit Sesi Kelas</h2>
        ...
        <form method="POST" action="{{ route('teacher.lms.update_session', $classSession) }}">
            ...
            <div class="mb-4">
                ...
                <select name="classroom_id" id="classroom_id" required
                    ...
                    @foreach ($classrooms as $id => $full_name)
                        <option value="{{ $id }}" {{ old('classroom_id', $classSession->classroom_id) == $id ? 'selected' : '' }}>
                            {{ $full_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="subject_name" id="subject_name" required
                    ...
                    @foreach ($subjects as $subject)
                        <option value="{{ $subject }}" {{ old('subject_name', $classSession->subject_name) == $subject ? 'selected' : '' }}>
                            {{ $subject }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="day_of_week" id="day_of_week" required
                    ...
                    @foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $day)
                        <option value="{{ $day }}" {{ old('day_of_week', $classSession->day_of_week) == $day ? 'selected' : '' }}>
                            {{ $day }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="time" name="start_time" id="start_time" 
                    value="{{ old('start_time', \Carbon\Carbon::parse($classSession->start_time)->format('H:i')) }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                @error('start_time')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <input type="time" name="end_time" id="end_time" 
                    value="{{ old('end_time', \Carbon\Carbon::parse($classSession->end_time)->format('H:i')) }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                @error('end_time')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
